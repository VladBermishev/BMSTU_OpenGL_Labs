...
class Lab3WindowInitParams : public GLGraphics::WindowInitParams {
public:
    Lab3WindowInitParams() = default;

    Lab3WindowInitParams(const std::string &title, const GLGraphics::Vector2ui &size) : GLGraphics::WindowInitParams(
            title, size) {}
...
int main() {
    Lab3WindowInitParams initialParams("ConycalCilinder", GLGraphics::Vector2ui(1000, 1000));
    Lab3Window window(initialParams);
    window.Run();
    return EXIT_SUCCESS;
}
//заполнения многоугольника по ребрам
//постфильтрация с взвешенным усреднением области 3x3